...
def buscar_sector( nombre: str, s1: dict, s2: dict, s3: dict, 
    ...
    """
    sector = dict
    if s1["nombre"].lower() == nombre:
        sector = s1
    elif s2["nombre"].lower() == nombre:
        sector = s2
    elif s3["nombre"].lower() == nombre:
        sector = s3
    elif s4["nombre"].lower() == nombre:
        sector = s4
...
def clasificar_sector( sector: dict ) -> str:
    ...
    if (sector["separador"] == False) and (sector["control_accesos"] == "Nulo"):
        #A2
        if (sector["pendiente"] <= 6) and (sector["ancho_calzada"] == 7.3) and (sector["ancho_berma"] == 1.8):
            tipo_de_sector = "A2"
        #B2
        elif (sector["pendiente"] <= 8) and (sector["ancho_calzada"] == 7.3) and (sector["ancho_berma"] == 1):
            tipo_de_sector = "B2"
        #C2
        elif (sector["pendiente"] <= 9) and (sector["ancho_calzada"] == 7) and (sector["ancho_berma"] == 0.5):
            tipo_de_sector = "C2"
        #D2
        elif (sector["pendiente"] <= 9) and (sector["ancho_calzada"] == 7) and (sector["ancho_berma"] == 0.4):
            tipo_de_sector = "D2"
    
    # VIA MULTICARRIL
    else:
        #A1
        if (sector["pendiente"] <= 5) and (sector["ancho_calzada"] == 7.3) and (sector["ancho_berma"] == 2.5):
            tipo_de_sector = "A1"
        #B1
        elif (sector["pendiente"] <= 6) and (sector["ancho_calzada"] == 7.3) and (sector["ancho_berma"] == 1.5):
            tipo_de_sector = "B1"
        
        #C1
        elif (sector["pendiente"] <= 8) and (sector["ancho_calzada"] == 7) and (sector["ancho_berma"] == 1.3):
            tipo_de_sector = "C1"
...
def determinar_velocidad_generica( sector: dict ) -> int:
    ...
    # Dos Carriles
    if (sector["separador"] == False) and (sector["control_accesos"] == "Nulo"):
        if (clasificar_sector(sector) == "A2") and (sector["peatones"] == False):
            velocidad_generica = 80
        elif (clasificar_sector(sector) == "A2") and (sector["peatones"] == True):
            velocidad_generica = 70
        if (clasificar_sector(sector) == "B2") and (sector["peatones"] == False):
            velocidad_generica = 70
        elif (clasificar_sector(sector) == "B2") and (sector["peatones"] == True):
            velocidad_generica = 60
        if (clasificar_sector(sector) == "C2") and (sector["peatones"] == True):
            velocidad_generica = 50
        elif (clasificar_sector(sector) == "D2") and (sector["peatones"] == True):
            velocidad_generica = 40

    # Multicarril
    else:
        if (clasificar_sector(sector) == "A1") and (sector["control_accesos"] == "Total") and (sector["peatones"] == False) and (sector["separador"] == True):
            velocidad_generica = 120
        elif (clasificar_sector(sector) == "B1") and (sector["control_accesos"] == "Parcial") and (sector["peatones"] == False) and (sector["separador"] == True):
            velocidad_generica = 100
        elif (clasificar_sector(sector) == "B1") and (sector["control_accesos"] == "Parcial") and (sector["peatones"] == False) and (sector["separador"] == False):
            velocidad_generica = 90
        elif (clasificar_sector(sector) == "B1") and (sector["control_accesos"] == "Nulo") and (sector["peatones"] == False) and (sector["separador"] == True):
            velocidad_generica = 90
        elif (clasificar_sector(sector) == "B1") and (sector["control_accesos"] == "Nulo") and (sector["peatones"] == False) and (sector["separador"] == False):
            velocidad_generica = 80
        elif (clasificar_sector(sector) == "C1") and (sector["control_accesos"] == "Nulo") and (sector["peatones"] == False) and (sector["separador"] == True):
            velocidad_generica = 80
        elif (clasificar_sector(sector) == "C1") and (sector["control_accesos"] == "Nulo") and (sector["peatones"] == False) and (sector["separador"] == False):
            velocidad_generica = 70
        elif (clasificar_sector(sector) == "C1") and (sector["control_accesos"] == "Nulo") and (sector["peatones"] == True) and (sector["separador"] == True):
            velocidad_generica = 70
        elif (clasificar_sector(sector) == "C1") and (sector["control_accesos"] == "Nulo") and (sector["peatones"] == True) and (sector["separador"] == False):
            velocidad_generica = 60
...
def contar_sitios_especiales( s1: dict, s2: dict, s3: dict, s4: dict ) -> dict:
    ...
    diccionario_con_suma_sitios_especiales["cuello_de_botella"] += zonas_recreativas("zona_escolar", s1)
    diccionario_con_suma_sitios_especiales["cuello_de_botella"] += zonas_recreativas("zona_escolar", s1)
    diccionario_con_suma_sitios_especiales["cuello_de_botella"] += zonas_recreativas("zona_escolar", s1)
    diccionario_con_suma_sitios_especiales["cuello_de_botella"] += zonas_recreativas("zona_escolar", s1)
...
def zonas_recreativas(zona: str, s: dict) -> str:
    if s.get(zona) == True:
        return 1
    else:
        return 0
