...
def cargar_blockchain_cupicoin(archivo: str) -> list:
    ...
    while bloque_comprimido != "":
        ...
        if bool(BlockChain_Cupicoin) == False:
            BlockChain_Cupicoin.append({
                "numero_bloque": bloque_comprimido_split[1],
        ...
        if int(bloque_pasado[1]) == int(bloque_comprimido_split[1]):
            BlockChain_Cupicoin[-1]["cantidad_transacciones"] += 1
        ...
        elif int(bloque_pasado[1]) != int(bloque_comprimido_split[1]):
...
def agregar_transaccion(BlockChain: list, transaccion: dict) -> None:
    ...
    """
    if (str(list(BlockChain[-1].keys())[-1])).isdigit() == True:
        BlockChain[-1][int((list(BlockChain[-1].keys())[-1]))+1] = transaccion # (list(BlockChain[-1].keys())[-1]) || Halla la ultima llave en el ultimo diccionario de la lista
    else:
        BlockChain[-1][0] = transaccion
...
def agregar_nuevo_bloque(BlockChain: list, timestamp: int) -> None:
    ...
    diccionario_final["timestamp"] = timestamp
...
# HALLAR EL HASH
def hash(Bloque: dict, timestamp: int) -> int:
    ...
    datos_concatenados += str(Bloque["numero_bloque"])+str(Bloque["hash_anterior"])
    
    for _ in datos_concatenados:
        SumaASCII += ord(_)
        
    return SumaASCII % int(timestamp)
...
def contar_veces_aparece_cuenta(BlockChain: list, direccion_de_cuenta: str) -> dict:
    ...
    for _ in BlockChain:
        i = 0
        if str((list(_.keys())[-1])).isdigit() == False:
            continue
        while i <= int((list(_.keys())[-1])):
...
def validar_bloque(BlockChain: list) -> bool:
    ...
    for _ in BlockChain:
        if (_["abierto"] == True) and (_["numero_bloque"] != BlockChain[-1]["numero_bloque"]) or (BlockChain[-1]["abierto"] != True):
            return False
        if (BlockChain[0]["hash_anterior"] != _["hash_anterior"]) and (_["hash_anterior"] != BlockChain[_["numero_bloque"]-1]["hash"]):
            return False 
